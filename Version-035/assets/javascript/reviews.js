...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadMedia() {
        fetch("/groupproject/Version-035/router/router.php?action=getMedia")
            .then(response => response.json())
            .then(data => {
                console.log("MEDIA FROM SERVER:", data);
    ...
    function applyFilters() {
        ...
        let filtered = mediaList.filter(item => {
            ...
            const typeMatch  = type === "all" || normalizeType(item.media_type) === normalizeType(type);
